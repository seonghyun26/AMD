...
function FolderIcon() {
  ...
  return (
    <svg viewBox="0 0 24 24" className="w-12 h-12 drop-shadow-sm" aria-hidden="true">
      <defs>
        <linearGradient id={gradientId} x1="2" y1="4" x2="22" y2="20" gradientUnits="userSpaceOnUse">
          <stop offset="0" stopColor="var(--amd-arc-cyan)" />
          <stop offset="0.3" stopColor="var(--amd-arc-sky)" />
          <stop offset="0.68" stopColor="var(--amd-arc-indigo)" />
          <stop offset="1" stopColor="var(--amd-arc-violet)" />
        </linearGradient>
      </defs>
      <path fill={`url(#${gradientId})`} d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z" />
    </svg>
  );
}
          ...
          >
            {creating ? (
              <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-blue-400 dark:border-blue-600 p-4 aspect-square">
                ...
                <div className="flex gap-3">
                  <button onClick={submit} className="amd-check-action"><Check size={16} /></button>
              ...
              <button
                ...
                className="amd-project-new group flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700 p-4 aspect-square transition-[border-color,box-shadow]"
              >
            ...
            {projects.map((p) => (
              <div
                ...
                className="amd-project-card group relative flex flex-col items-center justify-center gap-2 rounded-2xl p-4 aspect-square cursor-pointer transition-all"
              >
                <FolderIcon />
                {editingId === p.project_id ? (
                  <>
                    ...
                    />
                    <div className="flex gap-3">
                      <button onClick={(e) => { e.stopPropagation(); saveRename(p.project_id); }} className="amd-check-action"><Check size={15} /></button>
